...
class AlertRouter {
  ...
  send(type, severity, message, data = {}) {
    ...
    // Push notification via WebSocket for critical/high severity
    if (['critical', 'high'].includes(severity) && this.broadcast) {
      this.broadcast({ type: 'notification:push', data: alert });
    }
