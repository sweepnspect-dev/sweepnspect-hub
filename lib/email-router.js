...
const CATEGORIES = {
  ...
  tawk:     { severity: 'high',     autoCreate: null,          label: 'TAWK',     cls: 'tag-tawk' },
  facebook: { severity: 'high',     autoCreate: null,          label: 'FACEBOOK', cls: 'tag-facebook' },
...
const KEYWORD_MAP = [
  { keywords: ['live chat', 'chat:start', 'chat:end', 'chat transcript', 'tawk.to', 'tawk ticket'], category: 'tawk' },
  { keywords: ['fb message', 'facebook message', 'fb comment', 'facebook comment', 'facebook reaction'], category: 'facebook' },
...
// Sender-based routing (checked before keywords)
const SENDER_MAP = [
  { pattern: /tawkto\.email$/i, category: 'tawk' },
  { pattern: /tawk\.to$/i, category: 'tawk' },
  { pattern: /facebookmail\.com$/i, category: 'facebook' },
...
class EmailRouter {
  ...
  classify(email) {
    ...
    // 2. Bracket tag detection — check ALL bracket tags in subject
    //    e.g. [#6] 🟠 [FB MESSAGE] → finds [FB MESSAGE] → facebook
    //    e.g. [BUG] Camera freezes → finds [BUG] → bug
    const allBrackets = subject.match(/\[([^\]]+)\]/gi) || [];
    const bracketMap = {
      'live chat': 'tawk', 'chat': 'tawk', 'chat start': 'tawk', 'chat end': 'tawk',
      'fb message': 'facebook', 'fb comment': 'facebook', 'facebook': 'facebook',
      'webhook': 'ticket', 'bug': 'bug', 'bug report': 'bug',
      'ticket': 'ticket', 'billing': 'billing', 'founding': 'founding',
      'feature': 'feature', 'demo': 'demo', 'referral': 'referral',
    };
    for (const bracket of allBrackets) {
      const tag = bracket.slice(1, -1).toLowerCase();
      if (bracketMap[tag]) {
        const result = this._buildResult(bracketMap[tag]);
        this._cache.set(email.uid, result);
        return result;
      }
      if (CATEGORIES[tag]) {
    ...
    // 3. Bracket prefix (first word): [TICKET] ..., [BUG] ...
    ...
    // 4. Sender-based routing (fallback after bracket detection)
    //    tawkto.email with no bracket tag → tawk
    //    tawk.to company emails → tawk
    //    facebookmail.com → facebook
    for (const { pattern, category } of SENDER_MAP) {
      if (pattern.test(fromAddr)) {
