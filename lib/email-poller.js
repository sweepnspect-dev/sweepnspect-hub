...
class EmailPoller {
  constructor(broadcast, alertRouter) {
        ...
        if (this.checkCount > 0) {
          ...
          for (const msg of newMsgs) {
            ...
            this.broadcast({
              ...
              data: {
                type: 'email',
                text: `New email from ${msg.from.name || msg.from.address}: "${msg.subject}"`,
                timestamp: new Date().toISOString(),
              }
            });

            // Alert for founding-related emails
            const subj = (msg.subject || '').toLowerCase();
            const isImportant = subj.includes('founding') || subj.includes('application') ||
                                subj.includes('formsubmit') || subj.includes('sweepnspect');
            if (isImportant) {
              this.alertRouter.send('email-founding', 'high',
                `Founding email from ${msg.from.name || msg.from.address}: ${msg.subject}`,
