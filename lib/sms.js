...
class SmsService {
  ...
  /**
   * Speak a message aloud on the Z Fold via TTS.
   * Tries Termux TTS first, falls back to notification-only.
   */
  speakAlert(message) {
    if (!this.adbAvailable) return Promise.resolve({ spoke: false, reason: 'ADB not available' });

    const clean = message.replace(/'/g, '').replace(/"/g, '').replace(/\n/g, ' ').slice(0, 300);

    return new Promise((resolve) => {
      // Try Termux TTS (most reliable spoken output)
      const termuxCmd = `am startservice --user 0 -n com.termux.api/.TermuxApiService -a com.termux.tts_speak --es text '${clean}'`;
      execFile(this.adbPath, ['-s', this.zfoldAddr, 'shell', termuxCmd], { timeout: 10000 }, (err) => {
        if (!err) {
          console.log('[NOTIFY] TTS via Termux API');
          resolve({ spoke: true, method: 'termux-tts' });
        } else {
          // Fallback: Android media session approach (just log it — notification is primary)
          console.log('[NOTIFY] Termux TTS not available, notification only');
          resolve({ spoke: false, reason: 'Termux API not installed' });
        }
      });
    });
  }
  ...
  /**
   * Open a URL on the Z Fold via Via Browser.
   */
  openUrl(url) {
    if (!this.adbAvailable) return Promise.resolve({ opened: false, reason: 'ADB not available' });

    return new Promise((resolve) => {
      const shellCmd = `am start -n mark.via.gp/mark.via.Shell -a android.intent.action.VIEW -d '${url}'`;
      execFile(this.adbPath, ['-s', this.zfoldAddr, 'shell', shellCmd], { timeout: 10000 }, (err) => {
        if (!err) {
          console.log(`[NOTIFY] Opened URL on Z Fold: ${url.substring(0, 60)}`);
          resolve({ opened: true });
        } else {
          console.log(`[NOTIFY] Failed to open URL: ${err.message}`);
