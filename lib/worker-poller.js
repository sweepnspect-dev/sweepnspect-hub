...
class WorkerPoller {
  constructor(jsonStore, broadcast, alertRouter) {
    ...
    this.config = {
      ...
      pollInterval: parseInt(process.env.WORKER_POLL_INTERVAL || '30000'),
    };
  }
  ...
  _handleTicketCreate(evt) {
    ...
    this._archiveTawk({
      id: evt.chatId || 'tawk-' + Date.now(),
      chatId: evt.chatId || '',
      visitorName: ticket.name || 'Chat Visitor',
      visitorEmail: ticket.email || '',
      message: ticket.message || ticket.subject || 'Chat inquiry',
    ...
    this.broadcast({ type: 'tawk:message', data: { chatId: evt.chatId, name: ticket.name, ticketId: newTicket.id } });
  ...
  _handleChatStart(evt) {
    ...
    this._archiveTawk({
      ...
      message: evt.summary || `Chat started by ${visitor.name || 'Visitor'}`,
      time: evt.receivedAt || new Date().toISOString(),
      unread: true,
      type: 'chat-start',
      messages: [],
  ...
  _archiveTawk(msg) {
    try {
      const store = this.jsonStore('comms-tawk.json');
      ...
      console.error('[WORKER] Tawk archive error:', err.message);
    }
  }
